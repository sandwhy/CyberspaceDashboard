    ...
    <aside class="admin-sidebar" :class="{ 'collapsed': isSidebarCollapsed }">
      <div class="sidebar-header">
        <span v-if="!isSidebarCollapsed" class="logo-text"> 
          <img src="/logo/Cyberspace.png" alt="Cyberspace" class="logo-img" />
        </span>
      </div>
      ...
      <nav class="sidebar-menu">
        ...
        <NuxtLink to="/dashboard/schedules" class="metric-item" active-class="active" title="Schedules">
          <span class="menu-icon"><Calendar :size="20" /></span>
          <span v-if="!isSidebarCollapsed">Schedules</span>
        </NuxtLink>

        <NuxtLink to="/dashboard/datahub" class="metric-item" active-class="active" title="Data Hub">
          <span class="menu-icon"><Table2 :size="20" /></span>
          <span v-if="!isSidebarCollapsed">Data Hub</span>
        </NuxtLink>

        <div v-if="isPending" class="metric-item disabled" title="Access Pending">
          <v-icon size="20" color="grey">mdi-lock-clock</v-icon>
          <span v-if="!isSidebarCollapsed">Pending Approval</span>
        </div>
      </nav>
      ...
      <header class="admin-topbar">
        ...
        <div class="topbar-right">
          ...
          <div class="admin-profile">
            <div class="admin-avatar" :class="role.toLowerCase()">
              {{ userInitial }}
            </div>
...
<style scoped>
/* Keeping your existing styles, but adding logic for disabled items */
